  ...
  (some (fn [arg]
          (let [type (first arg)
            ...
            (when
              (some (fn [data]
                      (cond
                        (= type :tar) (verify-header data file 257)
                        :else
                        (if (instance? SignatureWithFooter data)
                          (verify-header-and-footer data file)
                          (verify-header data file))))
                    magic-number-set)
              type)))
        magic-numbers))

(defn detect-file-type [file]
  (cond
    (instance? InputStream file) (detect-type (.readAllBytes ^InputStream file))
    (bytes? file) (detect-type file)
    :else
    (logger/error "not an inputstream or byte array")))
